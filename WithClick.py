...
def interactive_click_plot(df, time_col, fx_col, fy_col, toe_y_col, file_path):
    global clicked_times
    clicked_times.clear()

    print("\n Click anywhere on the plot 4 times (Penetration Start → End, Extraction Start → End)")

    fig, ax1 = plt.subplots(figsize=(14, 7))
    ax2 = ax1.twinx()

    ax1.plot(df[time_col], df[fx_col], label='Force Fx', color='blue')
    ax1.plot(df[time_col], df[fy_col], label='Force Fz', color='red')
    ax2.plot(df[time_col], df[toe_y_col], label='Toe Position', color='orange')

    ax1.set_xlabel("Time (s)")
    ax1.set_ylabel("Force (N)")
    ax2.set_ylabel("Toe Position (m)")
    ax1.legend(loc='upper left')
    ax2.legend(loc='upper right')
    plt.title("Click: 1. Penetration Start → 2. End → 3. Extraction Start → 4. End")
    ...
    def on_click(event):
        ...
        ax1.text(time_val, ax1.get_ylim()[1]*0.9, str(len(clicked_times)), fontsize=20,
...
def review_and_confirm(df, time_col, fx_col, fy_col, toe_y_col, cut_times, file_path):
    ...
    for t, label, color in zip(cut_times, labels, colors):
        ...
        ax1.text(t, ax1.get_ylim()[1]*0.9, label, rotation=0, fontsize=20,
    ...
    ax1.legend(loc='upper left')
    ax2.legend(loc='upper right')
    plt.title("Review Selected Cut Points")
    ...
    review_filename = os.path.splitext(os.path.basename(file_path))[0]
    review_img_path = os.path.join(reviewplots_dir, f"review_{review_filename}.png")
    ...
    if cont == 'yes':
        ...
        interactive_click_plot(df, time_col, fx_col, fy_col, toe_y_col, file_path)

def save_cut_csvs(df, time_col, cut_times, file_path):
    t1, t2, t3, t4 = cut_times
    base = os.path.splitext(os.path.basename(file_path))[0]
    out_dir = r"C:\Users\Tarunika P\Desktop\Summer '25 Intern\USC\LASSIE\LASSIE_DataForHeatMap\LASSIE_DataForHeatMap\cut files"
    ...
    print(f"\n Penetration saved: {pen_path}")
    print(f" Extraction saved: {ext_path}")

def main():
    file_path = r"C:\Users\Tarunika P\Desktop\Summer '25 Intern\USC\LASSIE\LASSIE_DataForHeatMap\LASSIE_DataForHeatMap\plate1ex_angle45\plate1ex_angle45ex_len10_t5_Tue_Jun_17_15_31_40_2025.csv"
